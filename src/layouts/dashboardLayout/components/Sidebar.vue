<template>
	<div class="dashboard__sidebar" @mouseover="$emit('toggleDashboardSidebar')">
					...
					<g id="menu" transform="translate(0 -96.243)">
						<g id="Menu_1_">
							<path id="Path_87" data-name="Path 87"
								d="M12.03,120.3H372.939a12.03,12.03,0,0,0,0-24.06H12.03a12.03,12.03,0,1,0,0,24.06Z"
								fill="#ff4b57" />
							<path id="Path_88" data-name="Path 88"
								d="M372.939,180.455H12.03a12.03,12.03,0,1,0,0,24.06H372.939a12.03,12.03,0,1,0,0-24.06Z"
								fill="#ff4b57" />
							<path id="Path_89" data-name="Path 89"
								d="M372.939,264.667H132.333a12.03,12.03,0,1,0,0,24.06H372.939a12.03,12.03,0,0,0,0-24.06Z"
								fill="#ff4b57" />
						</g>
					</g>
				</svg>
		...
		<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical"
			@mouseover="toggleDashboardSidebar" @mouseleave="toggleDashboardSidebarMouseleave">
			<div v-for="(menu, index) in navMenuItems" :key="index" class="cursor-pointer">
				<router-link v-if="menu.url" :to="menu.url" class="nav-link" active-class="active"
				...
				<div v-else>
					<div class="nav-link my-1 cursor-pointer" :class="{ active: isToggleActive(menu, index) }"
						@click="toggleChildElems(index)">
						<div class="d-flex align-items-center">
							<div class="sidebar-icon-wrap">
								<span class="nav-link-icon">
									<img :src="menu.icon" class="img-fluid" alt="" />
					...
					<ul v-if="expandedItem === index">
						<li v-for="(childItem, index2) in menu.childrens" :key="index2" class="px-2xl-5 px-xl-3 px-2 ">
							<router-link v-if="childItem.url" :to="childItem.url" class="nav-link nav-child-link"
								active-class="active"
