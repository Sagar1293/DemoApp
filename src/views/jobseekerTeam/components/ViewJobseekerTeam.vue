			...
			<form class="tab-form">
				<div class="select-file">
					<div class="form_part form-row">
						<div class="form-group col-lg-4 col-12 theme-input">
							<label for="teamName">
							...
							<div class="flex position-relative">
								...
								>
									<ToolTip :title="t('toolTip.editJobSeekerTeamName')" position="bottom">
						...
						<!-- <div class="col-2">
							<span class="tbl-action grey-view">
								<FontAwesomeIcon :icon="editIcon" />
							</span>
						</div> -->
						...
						<div class="ml-auto">
							<ToolTip :title="t('toolTip.addTeamMember')">
								<div v-if="jobseekerTeamDetails.createdById !== seekerCheck.jobSeeker.id">
									<button type="button" class="btn-common btn-red" @click="addSeekerTeamMembers()" :disabled="true">
										{{ t('jobSeekerTeam.addMember') }}
									</button>
								</div>
								<div v-else>
									<button type="button" class="btn-common btn-red" @click="addSeekerTeamMembers()">
										{{ t('jobSeekerTeam.addMember') }}
									</button>
								</div>
							</ToolTip>
												...
												<div class="border-padding">
													<div v-if="i == newTableIndex" class="table-border">
														...
														<new-tb-tr>
															...
															<new-tb-td class="d-flex align-items-center justify-content-start">
																<ToolTip :title="t('toolTip.resendInvitation')">
																	<span>
																		<span
																			class="tbl-action red-close"
...
export default {
	...
	setup() {
		document.title = 'Edit Team - Way2Work';
		//ref
		...
		onMounted(() => {
			// edit invoice by id
			// sstore.dispatch('jobseekerTeam/viewJobseekerTeam', route.params.id);
			//jobseeker emp list
		...
		const jobseekerTeamInfo = computed(() => {
			...
			// sharedPartField.value = null;
			// shardTypeField.value = editSeekerTeamData.shared_type;
		...
		const onChange = event => {
			// teamMembersListNew.value.push(event)
		...
		const addSeekerTeamMembers = async () => {
			const values = {
				...
				// shared_part: sharedPartField.value,
				// shared_type: shardTypeField.value,
			};
			await store
		...
		const EditTeamNameDetail = val => {
			...
			console.log(editTeamDetails, 'hhhhh');
		};
