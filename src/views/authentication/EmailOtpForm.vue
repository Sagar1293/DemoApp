<template>
	...
	<section class="banner-image banner-section d-flex justify-content-center align-items-center">
		...
		<div class="login-box">
			...
			<a class="forgot d-flex align-items-center justify-content-center my-4" href="##">{{ t('emailOtpForm.login_email') }}</a>
			<form>
				...
				<a class="forgot-email d-flex align-items-center justify-content-center" @click="submitForm">
					<div class="btn-common btn-red btn-block text-center">{{ t('common.signIn') }}</div>
				</a>
			</form>
		</div>
	</section>
...
export default {
	...
	setup() {
		...
		// show forgotpassword
		const showForget1password = async () => {
			localStorage.setItem('changePassEmail', emailField.value);
			router.push({ name: 'ForgetPassword1Form' });
		};
