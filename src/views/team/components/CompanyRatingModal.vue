					...
					<form class="theme-input tab-form">
							...
							<div class="row">
								...
								>
									...
									<span class="text-red-500">{{
										...
										<div>
											<p class="text-grey fw-bolder ml-3">{{ star }} {{ t('companyRating.out_of_5') }}</p>
										</div>
									</div>
								</div>
							</div>
						...
						<div>
							<div class="form-group col-xl-4 col-lg-6 col-12 theme-input">
								<label for="teamName"> {{ t('companyRating.job') }}: </label>
								<div class="dropdown">
									<Multiselect
										class="form-group px-remove"
										label="job_title"
										@input="jobIdField.handleChange"
										@blur="jobIdField.handleBlur"
										v-model="jobIdField.value"
										:placeholder="
										...
										:searchable="true"
										valueProp="id"
										:options="leJobOptions"
										mode="single"
										:canDeselect="true"
									/>
									<p class="text-danger">
										{{ jobIdField.errorMessage }}
									</p>
								</div>
							</div>
							<p class="text-gray text-md fw-bold my-3">
								{{ t('companyRating.leave_review') }}
							</p>
							<textarea
								rows="4"
								class="explaination"
						...
						<div>
							<p for="reviewSuggestion" class="text-gray text-md fw-bold my-3">
								<!-- @click="suggetionText" -->
								{{ t('companyRating.suggestion') }}
							</p>
							<!-- <span v-if="suggestion"> -->
							<div class="d-flex my-md-0 my-2 gap-4">
								<span>
									<p class="ml-md-0 ml-2 title-xl-base" @click="suggetionValue('Neutral')">1) {{ t('seekerRating.neutral') }}</p>
								</span>
								<span>
									<p class="ml-md-0 ml-2 title-xl-base" @click="suggetionValue('Good')">2) {{ t('seekerRating.good') }}</p>
								</span>
								<span>
									<p class="ml-md-0 ml-2 title-xl-base" @click="suggetionValue('Very Good')">3) {{ t('seekerRating.very_good') }}</p>
								</span>
							</div>
							<!-- </span> -->
						</div>
						...
						"
						>
							<button type="button" class="btn-common btn-red mt-2 text-center" @click="submitForm">
								{{ t('job.add') }}
							</button>
						</div>
					</form>
...
export default {
	...
	setup(props, { emit }) {
		...
		const jobIdField = reactive(useField('jobId', undefined, { initialValue: null }));
		...
		const submitForm = handleSubmit(async values => {
			// form data
			values.seekerId = route.params.id ?? null;
		...
		const suggetionText = () => {
			// commentField.value = suggestion.value;
