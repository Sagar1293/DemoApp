<template>
				...
				<div class="mgb-row">
					...
					<div class="ml-auto d-md-flex align-items-center">
						...
						<div class="d-flex align-items-center justify-content-center mt-xl-0 mt-2">
							...
							<!-- <div class="ml-lg-3 m-auto grey-edit yellow-star" v-if="job.job_status == 'completed' && job.seeker_rating == null" @click="ratingReviewModal = job.id">
								<button>
									<FontAwesomeIcon :icon="startIcon" />
								</button>
							</div>
							<div class="ml-3 grey-edit ml-2" v-else-if="job.job_status == 'completed' && job.seeker_rating != null">
								<button>
									<FontAwesomeIcon :icon="startIcon" />
								</button>
							</div> -->
	...
	<!-- <Suspense v-if="ratingReviewModal.value">
		<template #default>
			<rating-review-modal v-model:isModalShow="ratingReviewModal" @newDocumentUploaded="fetchCompanyJobList" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
...
export default {
	...
	setup() {
		...
		// const totalPages = computed(() => store.state.job.totalCount);
