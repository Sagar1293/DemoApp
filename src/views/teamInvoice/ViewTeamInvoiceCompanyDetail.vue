	...
	<div class="document__list card-space container-lg">
		<div class="">
			<div class="row justify-content-center">
				<!-- <div class="col-12">
					<div class="d-flex align-items-center justify-content-end" @click="teamInvoicePdf(seekerTeamInvoiceDetail)">
					...
					<div class="red-wrapper">
						<div class="pb-4 d-flex justify-content-between">
							<h5 class="title-head">
								...
								{{ t('leInvoice.viewInvoice.invoice') }}
							</h5>
						...
						<div class="red-box">
							...
							<div class="mb-2" v-if="seekerTeamInvoiceDetail.team_invoice_company && seekerTeamInvoiceDetail.team_invoice_company.reg_number">
								<h5 class="company-value title-lg">{{ t('leInvoice.viewInvoice.comapanyId') }}</h5>
								<p class="text-md-fw">
									{{ seekerTeamInvoiceDetail.team_invoice_company && seekerTeamInvoiceDetail.team_invoice_company.reg_number }}
								</p>
							</div>
							...
							<!-- <div class="mb-2">
								<p class="company-value title-lg">Comapany ID</p>
								<p class="text-md-fw">
									{{ seekerTeamInvoiceDetail.companyId }}
								</p>
							</div> -->
						</div>
					</div>
			...
			<div class="mt-4">
				<div class="bg-white pb-3 mt-10">
					<div class="employer-details flex justify-between items-center">
						<div class="pph-left">
							<h3 class="title-head">{{ t('leInvoice.viewInvoice.seekerWHList') }}</h3>
						</div>
					</div>
							...
							<div class="daily-work-col">
								...
								<div class="details-head text-center flex items-center text-white">
									<div class="member-name">
										<p class="details-head-text text-start font-bold">{{ t('leInvoice.viewInvoice.seekerName') }}</p>
									</div>

									<div class="role-member">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.Role') }}</p>
									</div>

									<div class="email-address">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.emailAddress') }}</p>
									</div>

									<div class="share-value">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.Share') }}</p>
									</div>

									<div class="Tax-value">
										<p class="details-head-text font-bold">{{ t('leInvoice.viewInvoice.taxPercentage') }}</p>
									</div>

									<div class="allowances">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.Allowances') }}</p>
									</div>
									<div class="payment">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.willBePaid') }}</p>
									</div>
									<div class="actions">
										<p class="details-head-text font-bold">{{ t('teamInvoiceList.Actions') }}</p>
									</div>
								</div>

								<!-- paymentsharing list - row -->
								<!-- :data="seekerTeamInvoiceDetail.invoice_seekers || []" sst -->
								<div v-for="(item, index) in seekerTeamInvoiceDetail.invoice_seekers || []" :key="index">
									<div class="details-content text-center">
										...
										<div class="payment">
											<div>
												<p class="text-red">€ {{ item.sub_total.toFixed(2) }}</p>
											</div>
										</div>
									...
									<div class="opne-toggle p-3" v-if="index === expandRowIndex">
										...
										<div class="details-head text-center text-white">
											<div class="product-no">
												<p class="details-head-text font-bold">{{ t('leInvoice.viewInvoice.invoiceDetails') }}</p>
											</div>
											...
											<div class="contract-type">
												<p class="details-head-text font-bold">{{ t('leInvoice.addInvoice.vat') }}%</p>
											</div>
											<div class="contract-type">
												<p class="details-head-text font-bold">{{ t('leInvoice.viewInvoice.noVat') }}</p>
											</div>
											<div class="contract-type">
												<p class="details-head-text font-bold">{{ t('leInvoice.addInvoice.vat') }}</p>
											</div>
										...
										<div v-for="(itemRaw, index) in item.workHours || []" :key="index">
											<div class="row no-gutters border-box">
												<div class="details-content">
													...
													<!-- vat % -->
													<div class="hours-width me-2 text-center">
														<p>{{ seekerTeamInvoiceDetail.vat || '-' }}</p>
													</div>
													<!-- no vat -->
													<div class="hours-width me-2 text-center">
														<p>€{{ itemRaw.contract_value * itemRaw.total_work || '-' }}</p>
													</div>
													<!-- vat -->
													<div class="hours-width me-2 text-center">
														<p>€ {{ (itemRaw.contract_value * itemRaw.total_work * seekerTeamInvoiceDetail.vat) / 100 }}</p>
													</div>
													<!-- contract value -->
													<div class="contractvalue-width text-center">
														<p>€ {{ itemRaw.contract_value * itemRaw.total_work + (itemRaw.contract_value * itemRaw.total_work * seekerTeamInvoiceDetail.vat) / 100 }}</p>
													</div>
												</div>
											</div>
											...
											<div class="ml-auto total mt-3 mr-3">
												<div>
													<div class="total-excl d-flex justify-content-between w-full pb-2">
														<p class="text-total">{{ t('leInvoice.viewInvoice.totalExclVat') }}</p>
													...
													<div class="total-excl border-top-bottom d-flex justify-content-between font-semibold text-right text-lg py-2">
														...
														<p class="total-prize">€ {{ item.sub_total.toFixed(2) || '0' }}</p>
													</div>
												</div>
											</div>
											<div class="bottom-header d-md-flex d-block align-items-center justify-content-between mb-4">
												...
												<div class="d-flex align-items-center ml-md-auto">
													<div class="mr-5 text-white">
														...
														<p class="text-sm-fw">€ {{ (item.seeker_company_pay && item.seeker_company_pay.toFixed(2)) || 0.0 }}</p>
													</div>

													<div class="mr-5 text-white">
														<p class="text-sm-fw">{{ t('leInvoice.addInvoice.VATAmount') }}</p>
														<p class="text-sm-fw">€ {{ (item.worked_value && item.worked_value.toFixed(2)) || '-' }}</p>
													</div>

													<!-- <div class="mr-5 text-white">
							<p class="text-md-fw">Gross amount</p>
							<p class="text-md-fw">€ {{ Number(seekerTeamInvoiceDetail.gross_salary).toFixed(2) || '-' }}</p>
						</div>

						<div class="mr-5 text-white">
							<p class="text-md-fw">Withholding tax</p>
							<p class="text-md-fw">€ {{ Number(seekerTeamInvoiceDetail.tax_amount).toFixed(2) || 0.0 }}</p>
						</div>

						<div class="mr-5 text-white">
							<p class="text-md-fw">Compensations</p>
							<p class="text-md-fw">€ {{ Number(seekerTeamInvoiceDetail.compensation_amount).toFixed(2) || '-' }}</p>
						</div>

						<div class="text-white">
							<p class="text-md-fw">You will be paid</p>
							<p class="text-md-fw">€ {{ Number(seekerTeamInvoiceDetail.sub_total).toFixed(2) || '-' }}</p>
						</div> -->
												</div>
											</div>
										</div>
				...
				<new-data-table :data="seekerTeamInvoiceDetail.invoice_files || []" sst class="large-table">
					<template #thead>
						<new-tb-th class="w-50">{{ t('leInvoice.viewInvoice.placeOfPurchase') }}</new-tb-th>
						<new-tb-th class="w-30">{{ t('leInvoice.viewInvoice.dateOfPurchase') }}</new-tb-th>
					...
					<div class="viewinvoicetable">
						<div class="d-flex">
							...
							<div style="width:33%; border-right:2px solid #000;border-bottom:2px solid #000;" class="p-2">
								...
								<p class="ml-2">{{ seekerTeamInvoiceDetail.created_by && seekerTeamInvoiceDetail.created_by.seeker && seekerTeamInvoiceDetail.created_by.seeker.account_number }}</p>
							</div>
							<div style="width:55%; border-bottom:2px solid #000;" class="p-2">
								<p>{{ t('leInvoice.viewInvoice.bic') }}</p>
								<p class="ml-2">
									{{ t('leInvoice.viewInvoice.bank_bic') }} :
									{{ seekerTeamInvoiceDetail.created_by && seekerTeamInvoiceDetail.created_by.seeker && seekerTeamInvoiceDetail.created_by.seeker.bic_number }}
								</p>
							</div>
						</div>
						...
						<div class="d-flex">
							...
							<div style="width: 33%; border-bottom:2px solid #000; border-right:2px solid #000;" class="p-2">{{ t('leInvoice.viewInvoice.way2_work_oy') }}</div>
						...
						<div class="d-flex">
							<div style="width:12%; border-right:2px solid #000;" class="p-2 d-flex flex-column justify-content-between align-items-start position-relative">
								<div class="text-uppercase verticle-text ml-2">{{ t('leInvoice.viewInvoice.tillisirato_girering') }}</div>
								<div class="ml-auto mb-5 table-text">{{ t('leInvoice.viewInvoice.maksaja_betalare') }}</div>
								<div class="text-end">{{ t('leInvoice.viewInvoice.allekirjoitus_underskrift') }}</div>
							</div>

							<div class="d-flex flex-column justify-content-between p-2" style="width:33%; border-right:2px solid #000;">
								<div class="p-2">
									{{ t('leInvoice.viewInvoice.customer_details') }}
									<p>
										{{ t('leInvoice.viewInvoice.full_name') }} :
										{{
											seekerTeamInvoiceDetail.created_by &&
												seekerTeamInvoiceDetail.created_by.seeker &&
												seekerTeamInvoiceDetail.created_by.seeker.user &&
									...
									<p>
										{{ t('leInvoice.viewInvoice.email') }} :
										{{
											seekerTeamInvoiceDetail.created_by &&
												seekerTeamInvoiceDetail.created_by.seeker &&
												seekerTeamInvoiceDetail.created_by.seeker.user &&
												seekerTeamInvoiceDetail.created_by.seeker.user.email
										}}
									</p>
									<p>
										{{ t('leInvoice.viewInvoice.phone_number') }} :
										{{
											seekerTeamInvoiceDetail.created_by &&
												seekerTeamInvoiceDetail.created_by.seeker &&
												seekerTeamInvoiceDetail.created_by.seeker.user &&
												seekerTeamInvoiceDetail.created_by.seeker.user.mobile
										}}
									</p>
									<p>
										{{ t('leInvoice.viewInvoice.gender') }} :
										{{
											seekerTeamInvoiceDetail.created_by &&
												seekerTeamInvoiceDetail.created_by.seeker &&
												seekerTeamInvoiceDetail.created_by.seeker.user &&
												seekerTeamInvoiceDetail.created_by.seeker.user.gender
										}}
									</p>
									<p>
										{{ t('leInvoice.viewInvoice.dob') }} :
										{{
											seekerTeamInvoiceDetail.created_by &&
												seekerTeamInvoiceDetail.created_by.seeker &&
												seekerTeamInvoiceDetail.created_by.seeker.user &&
												dateFormat(seekerTeamInvoiceDetail.created_by.seeker.user.dob)
										}}
									</p>
								</div>
							...
							<div style="width:55%;border-top:2px solid #000;" class="d-flex align-items-end">
								<div style="width:24%; border-right:2px solid #000;border-top:2px solid #000;" class="p-2">{{ t('leInvoice.viewInvoice.vitenro_ref_nr') }}</div>
								<div style="width:86%;border-top:2px solid #000;" class="p-2">{{ t('leInvoice.viewInvoice.reference_number') }} : {{ seekerTeamInvoiceDetail.ref_number }}</div>
							</div>
						</div>
						...
						<div style="border-top:2px solid #000;">
							<div class="d-flex">
								...
								<div class="p-2" style="width:10%;border-right:2px solid #000; border-bottom:2px solid #000;">{{ t('leInvoice.viewInvoice.erapaiva_forf_dag') }}</div>
								<div class="p-2 d-flex justify-content-center align-items-center" style="width:20%;border-right:2px solid #000; border-bottom:2px solid #000;">
									{{ t('leInvoice.viewInvoice.due_date') }} :
									{{ dateFormat(seekerTeamInvoiceDetail.due_date) }}
								</div>
								<div class="p-2" style="width:25%; border-bottom:2px solid #000;">
									<p>{{ t('leInvoice.viewInvoice.summa') }}</p>
									<p class="text-center">{{ t('leInvoice.viewInvoice.total') }} : {{ seekerTeamInvoiceDetail.sub_total }}</p>
								</div>
							</div>
						</div>
						...
						<div class="d-flex align-items-center my-3">
							<div class="barcode-text">
								<p class="d-flex align-items-center justify-content-center">{{ t('leInvoice.viewInvoice.barcode_here') }}</p>
							</div>
							<div>
								<p class="title-sm">
...
export default {
	...
	setup() {
		...
		document.title = 'View team invoice - Way2Work';
		//ref
		const expandRowIndex = ref(null);

		// const dummyArray = ref[1,2];

		// const seekerTeamInvoiceDetail = ref(null);
		...
		onMounted(() => {
			...
			// fetchseekerTeamInvoiceDetail();
		});

		// const fetchseekerTeamInvoiceDetail = () => {
		// 	seekerTeamInvoiceDetail.value = store.state.invoice.invoiceSeekerDetail;
		// };
		...
		// const seekerTeamInvoiceDetail = computed(()=>store.state.invoice.invoiceSeekerDetail)
		...
		// 		totalMealBenefits.value +
		// 		totalFullPerDiem.value +
		// 		totalOtherBenefits.value +
		// 		seekerTeamInvoiceDetail.tax_amount +
		// 		seekerTeamInvoiceDetail.compensation_amount +
		// 		seekerTeamInvoiceDetail.deduction;
		// 	return fTotal;
		// });
		/**
		 * Watch
