<template>
	<div class="invoice__list card-space">
		...
		<div class="row" v-if="userTypeInfo == 'jobseeker'">
			<!-- header -->
			<div class="payment-pill-header flex-sm-nowrap flex-wrap">
				<div class="d-flex align-items-center justify-content-between">
					<div class="pph-left">
						<h5 class="title-lg">
							{{ t('teamInvoiceList.titleSeeker') }}
						</h5>
					</div>
					<div class="select-drop mt-xl-2 ms-xl-4 ms-md-3 ms-2">
				...
				<ToolTip :title="t('toolTip.addTeamInvoice')">
					<div class="pph-right d-flex align-items-center justify-content-center ms-md-0 ms-sm-2 ms-0">
						<router-link to="/team-invoices/add" class="btn-common btn-red">+ {{ t('teamInvoiceList.add') }} </router-link>
					</div>
				</ToolTip>
			</div>
			...
			>
				<template #header>
					<div class="select__limit">
						<span>{{ t('teamInvoiceList.show') }}</span>
						<div class="dropdown">
							<a
								class="
									dropdown-toggle
						...
						<span class="me-sm-3">{{ t('teamInvoiceList.entries') }}</span>
						<div class="d-md-flex align-items-center select-date-range mb-xl-0 mb-2">
							<div class="mb-lg-0 mb-2 w-70">
								<label for="dateRange"> {{ t('leInvoice.invoiceList.selectDateRange') }}</label>

								<!-- <v-date-picker v-model="dateRangeSelect.value" :masks="{ input: 'DD.MM.YYYY' }" is-range> </v-date-picker> -->
								<v-date-picker @input="dateRangeSelect.handleChange" @blur="dateRangeSelect.handleBlur" v-model="dateRangeSelect.value" :masks="{ input: 'DD.MM.YYYY' }" is-range>
									<template v-slot="{ inputValue, inputEvents }">
										<div class="flex justify-center items-center">
											<input :value="inputValue.start" v-on="inputEvents.start" class="border px-2 py-1 w-30 radius-full focus:outline-none focus:border-indigo-300" readonly />
											<svg class="w-3 mx-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
												<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
											</svg>
											<input :value="inputValue.end" v-on="inputEvents.end" class="border px-2 py-1 w-30 radius-full focus:outline-none focus:border-indigo-300" readonly />
										</div>
									</template>
								</v-date-picker>
								<!-- <v-date-picker v-model="dateRangeSelect.value" is-range /> -->
							</div>
							<div class="mr-2 mb-lg-0 mb-2 w-100">
				...
				<template #thead>
					...
					<new-tb-th sort-key="due_date">{{ t('teamInvoiceList.dueDate') }}</new-tb-th>
					<new-tb-th sort-key="invoice_number">{{ t('teamInvoiceList.invoiceId') }}</new-tb-th>
					<new-tb-th>{{ t('teamInvoiceList.teamName') }}</new-tb-th>
					<!-- <new-tb-th>Invoice Date</new-tb-th> -->
					<new-tb-th sort-key="company_name">{{ t('teamInvoiceList.invoiceTo') }}</new-tb-th>
					<new-tb-th>{{ t('teamInvoiceList.totalInvoice') }}</new-tb-th>
					<!-- <new-tb-th>Downloads</new-tb-th> -->
					<new-tb-th sort-key="approved">{{ t('teamInvoiceList.status') }}</new-tb-th>
					<new-tb-th sort-key="payment_status">{{ t('teamInvoiceList.payment_status') }}</new-tb-th>
							...
							<div v-else>
								...
								<div class="tbl-status" v-else>
									<span class="sts-approve d-flex align-items-center justify-content-center">
										...
										<p>{{ t('teamInvoiceList.Approved') }}</p>
									</span>
								</div>
							</div>
						</new-tb-td>
						<new-tb-td>
							<div class="tbl-status">
								...
								<span class="sts-wait  d-flex align-items-center justify-content-center" v-else>
									...
									<p>{{ t('teamInvoiceList.partiallyPaid') }}</p>
								</span>
							</div>
						</new-tb-td>
						<new-tb-td>
			...
			<div>
				...
				>
					<template #header>
						<div class="select__limit ">
							<div class="d-flex justify-content-center mb-sm-0 mb-2 align-items-center">
								<span>{{ t('teamInvoiceList.show') }}</span>
								<div class="dropdown">
									<a
										class="
							...
							<div class="d-md-flex align-items-center select-date-range mb-xl-0 mb-2">
								<div class="mb-md-0 mb-2 w-100">
									...
									<v-date-picker @input="dateRangeSelect.handleChange" @blur="dateRangeSelect.handleBlur" v-model="dateRangeSelect.value" :masks="{ input: 'DD.MM.YYYY' }" is-range>
										<template v-slot="{ inputValue, inputEvents }">
											<div class="flex justify-center items-center">
												<input :value="inputValue.start" v-on="inputEvents.start" class="border px-2 py-1 w-30 radius-full focus:outline-none focus:border-indigo-300" />
												<svg class="w-3 mx-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
												</svg>
												<input :value="inputValue.end" v-on="inputEvents.end" class="border px-2 py-1 w-30 radius-full focus:outline-none focus:border-indigo-300" />
											</div>
										</template>
									</v-date-picker>
									<!-- <v-date-picker v-model="dateRangeSelect.value" is-range /> -->
							...
							<div>
								...
								<div class="border-padding">
									<div v-if="i == tableIndex" class="table-border">
										<new-tb-tr>
											<new-tb-th>{{ t('teamInvoiceList.invoiceId') }}</new-tb-th>
										...
										<new-tb-tr>
											<new-tb-th>{{ t('teamInvoiceList.invoiceTo') }}</new-tb-th>
										...
										<new-tb-tr>
											<new-tb-th>{{ t('teamInvoiceList.status') }}</new-tb-th>
										...
										<new-tb-tr>
											<new-tb-th>{{ t('teamInvoiceList.payment_status') }}</new-tb-th>
		...
		<div class="row" v-if="userTypeInfo == 'company'">
			<div class="table-header">
				...
				<div class="select__limit mobile-table-header">
					...
					<span>{{ t('teamInvoiceList.entries') }}</span>
				</div>
			</div>
			...
			<new-data-table
				...
				:max-items="maxItemsPerPage"
				@change-page="handleChangePage"
				class="large-table table-overflow"
			>
				<template #header>
					<div class="select__limit">
							...
							<div class="d-flex">
								...
								<div>
									...
									<div v-else class="d-flex align-items-center justify-content-end">
										<!-- copywSeekerInvoiceDetail(tr.id) -->
										<ToolTip :title="t('toolTip.markAsWrong')">
											<button class="tbl-action grey-edit-grey ml-2">
												<FontAwesomeIcon :icon="thumbsUp" style="text-align: 'center'" />
											</button>
...
export default {
	...
	setup() {
		...
		if (userTypeInfo.value == 'jobseeker') {
			document.title = 'Team Invoicing - Way2Work';
		} else {
			document.title = 'All Team Invoicing - Way2Work';
		}
		/**
		 * Methods
		 * *******************************************************************
		...
		const updateTeamMarkAsWrong = (invoiceId, flag) => {
			...
			// Swal.fire({
			// 	title: 'Are you sure?',
			// 	text: 'You want to mark as wrong this invoice ?',
			// 	icon: 'warning',
		...
		//remove value - Status
		// const removeStatus = event => {
		// 	if (event) {
		// 		invoiceStaus.value = null;
		// 	}
		// };

		// remove value - Group List
		// const removeGroupList = event => {
		// 	if (event) {
		// 		teamId.value = null;
		// 	}
		// };
