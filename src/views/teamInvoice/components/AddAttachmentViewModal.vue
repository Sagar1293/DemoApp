		...
		<div class="model">
			...
			<div>
				<div class="select-file">
					...
					<div class="form-row account__form">
						<div class="form-group col-xl-3 col-lg-6">
							<label for="firstName">
								<span class="input-title">place of Purchase</span>
							</label>
							<input id="viewPlaceOfPurchase" type="text" class="form-control form-model" />
						</div>

						<div class="form-group col-xl-3 col-lg-6">
							<label for="firstName">
								<span class="input-title">Date of Purchase</span>
							</label>
							<input id="viewDateOfPurchase" type="text" class="form-control form-model" />
						</div>

						<div class="form-group dropdown select-employee col-xl-2 col-lg-6">
							<div>
								<label for="vatselect" class="w-full">
									<span class="input-title">VAT%</span>
								</label>
								<button class="" type="button" id="viewVat" data-bs-toggle="dropdown" aria-expanded="false">
									<FontAwesomeIcon :icon="downArrowIcon" class="ms-3 text-gray" />
								</button>
						...
						<div class="col-xl-5">
							...
							<div>
								...
								<div class="preview" v-if="invoiceReceiptFile">
									<button type="button" class="drop-close" @click="removeInvoiceReceiptFile">
										<FontAwesomeIcon :icon="closeIcon" />
									</button>
									<div id="preview">
										<div class="dz-preview dz-error dz-complete dz-image-preview">
											<div class="dz-image">
												<img data-dz-thumbnail="" alt="Invocie receipt file image" :src="invoiceReceiptFilePreview" class="thumb-img" />
											</div>
				...
				<div class="text-center">
					<button class="btn-common btn-red" @click="closeModal">Done</button>
				</div>
			</div>
		</div>
	</vue-final-modal>
...
export default {
	...
	setup(props) {
		...
		const companyAddressField = reactive(useField('address', undefined, { initialValue: null }));
		const companyPhoneField = reactive(useField('company_phone', undefined, { initialValue: null }));
		const employerSSNField = reactive(useField('ssn', undefined, { initialValue: null }));
		const employerTaxNumberField = reactive(useField('tax_number', undefined, { initialValue: null }));
		...
		const submitForm = handleSubmit(async (values) => {
			await store.dispatch('auth/addAnonymousEmployer', values).then(() => {
