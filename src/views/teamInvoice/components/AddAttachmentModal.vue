						...
						<div class="invoice-pd invoice-receipt">
							<div class="row">
								<div class="d-md-flex justify-content-between mb-3 mt-5 ">
									<div class="pph-left">
										<h5 class="title-lg mb-2">{{t('invoiceData.attachments')}}</h5>
									</div>
...
export default {
	...
	setup(props) {
		...
		const handleInvoiceMultiFile = acceptedFiles => {
			...
			// file preview
			invoiceMultiFilePreview.value = ['image/png'].includes(acceptedFiles[0].type) ? URL.createObjectURL(acceptedFiles[0]) : require('@/assets/images/iconWomanDraw.png');
		};
