					...
					<div class="red-wrapper">
						...
						<div class="red-box">
							...
							<div class="mb-2" v-if="seekerTeamInvoiceDetail.team_invoice_company && seekerTeamInvoiceDetail.team_invoice_company.reg_number">
								<p class="company-value title-lg">{{ t('leInvoice.viewInvoice.comapanyId') }}</p>
								<p class="text-md-fw">
			...
			<div>
				<div class="bg-white pb-3 mt-10">
					<div class="employer-details flex justify-between items-center">
						<div class="pph-left">
							<h3 class="font-semibold">{{ t('leInvoice.viewInvoice.seekerWHList') }}</h3>
						</div>
					</div>
									...
									<div class="opne-toggle p-3" v-if="index === expandRowIndex">
										...
										<div class="overflow-auto">
											<div class="bottom-header d-flex align-items-center justify-content-between my-3">
												<div class="pph-left">
													<h5 class="text-white fn16">{{ t('leInvoice.addInvoice.summary') }}</h5>
												</div>
												<div class="d-flex align-items-center ml-md-auto">
													<div class="mr-5 text-white">
														<p class="text-md-fw">{{ t('leInvoice.addInvoice.grossAmount') }}</p>
														<p class="text-md-fw">€ {{ item.gross_salary ? Number(finalTotal.grossSalary).toFixed(2) : 0 }}</p>
														<!-- {{ GrossSalary ? GrossSalary : 0 }} -->
													</div>

													<div class="mr-5 text-white">
														<p class="text-md-fw">{{ t('leInvoice.addInvoice.taxAmount') }}</p>
														<p class="text-md-fw">€ {{ item.tax_amount ? Number(finalTotal.taxbledAmount).toFixed(2) : 0 }}</p>
														<!-- {{ TaxbledAmount ? TaxbledAmount : 0 }} -->
													</div>

													<div class="mr-5 text-white">
														<p class="text-md-fw">{{ t('leInvoice.addInvoice.compensations') }}</p>
														<p class="text-md-fw">€ {{ Number(item.compensation_amount).toFixed(2) || 0 }}</p>
														<!-- {{ Number(compensationAmount).toFixed(2) || 0 }} -->
													</div>

													<div class="text-white">
														<p class="{{'text-md-fw'|translate}}">{{ t('leInvoice.addInvoice.willpaid') }}</p>
														<p class="text-md-fw">€ {{ Number(item.seeker_company_pay).toFixed(2) || 0 }}</p>
					...
					<div class="bottom-header d-flex align-items-center justify-content-between my-3">
						...
						<div class="d-flex align-items-center ml-md-auto">
							<div class="mr-5 text-white">
								...
								<p class="text-md-fw">€ {{ finalTotal.grossSalary ? Number(finalTotal.grossSalary).toFixed(2) : 0 }}</p>
							</div>

							<div class="mr-5 text-white">
								<p class="text-md-fw">Tax amount</p>
								<p class="text-md-fw">€ {{ finalTotal.taxbledAmount ? Number(finalTotal.taxbledAmount).toFixed(2) : 0 }}</p>
							</div>

							<div class="mr-5 text-white">
								<p class="text-md-fw">Compensations</p>
								<p class="text-md-fw">€ {{ Number(finalTotal.compensationAmount).toFixed(2) || 0 }}</p>
							</div>

							<div class="text-white">
								<p class="{{'text-md-fw'|translate}}">You will be paid</p>
								<p class="text-md-fw">€ {{ Number(finalTotal.youWillBePaid).toFixed(2) || 0 }}</p>
							</div>
						</div>
					</div>
				</div> -->
					...
					<div class="bottom-header d-flex align-items-center justify-content-between mb-4">
						...
						<div class="d-flex align-items-center ml-md-auto">
							<div class="mr-5 text-white">
								...
								<p class="text-md-fw">€ {{ Number(finalTotal.invoiceTotal).toFixed(2) || 0.0 }}</p>								
							</div>

							<div class="mr-5 text-white">
								<p class="text-md-fw">VAT Amount</p>
								<p class="text-md-fw">$ {{ finalTotal.vatTotal || 0 }}</p>								
							</div>
						</div>
					</div>
				</div> -->
					...
					<div class="viewinvoicetable">
						<div class="d-flex">
							...
							<div style="width:55%; border-bottom:2px solid #000;" class="p-2">
								<p>BIC</p>
								<p class="ml-2">
									Bank BIC : {{ seekerTeamInvoiceDetail.created_by && seekerTeamInvoiceDetail.created_by.seeker && seekerTeamInvoiceDetail.created_by.seeker.bic_number }}
								</p>
							</div>
						</div>
						...
						<div class="d-flex">
							...
							<div style="width:55%;border-top:2px solid #000;" class="d-flex align-items-end">
								<div style="width:24%; border-right:2px solid #000;border-top:2px solid #000;" class="p-2">
									Vitenro Ref. nr
								</div>
								<div style="width:86%;border-top:2px solid #000;" class="p-2">Reference Number : {{ seekerTeamInvoiceDetail.ref_number }}</div>
							</div>
						</div>
						...
						<div style="border-top:2px solid #000;">
							<div class="d-flex">
								...
								<div class="p-2" style="width:25%; border-bottom:2px solid #000;">
									<p>Summa</p>
									<p class="text-center">Total : {{ seekerTeamInvoiceDetail.sub_total }}</p>
								</div>
							</div>
						</div>
		...
		const finalTotal = computed(() => {
			...
			return (
				store.state.teamInvoice?.teaminvoiceSeekerDetail?.invoice_seekers?.reduce((acc, item) => {
					// const memberTotal = [seekerTeamInvoiceDetail.value?.created_by?.seeker?.id, item.seeker.id].includes(userDetail.value?.jobSeeker?.id);
					// if (memberTotal) {
