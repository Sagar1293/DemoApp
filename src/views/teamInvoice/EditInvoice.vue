	...
	<div class="invoice__list card-space">
		...
		<ToolTip :title="t('toolTip.updateTeam')">
			<div class="text-center mt-5 pb-3">
				<button class="btn-common btn-red" @click="updateInvoice" :disabled="paymentSharingRef?.teamMembers.length === 0 || paymentSharingRef?.leftToShareAmount !== 0">
					{{ t('common.update') }}
				</button>
			</div>
		</ToolTip>
...
export default {
	...
	setup() {
		document.title = 'Edit team invoice - Way2Work';
		// ref
		...
		const invoiceSchema = yup.object({
			...
			reference_number: yup
				...
				// .required(
				// 	t('validation.requiredField', {
				// 		field: 'fieldList.reference_number',
				// 	}),
				// )
				.min(2),
			invoice_date: yup
		...
		onMounted(() => {
			...
			store.dispatch('teamInvoice/editTeamInvoice', route.params.id).then(({ data }) => {
				// const edigtInvoiceData = store.state.teamInvoice.editTeamWorkDetailList;
		...
		const updateInvoice = handleSubmit(async values => {
			...
			values.team_worked_hours = store.state.teamInvoice.editTeamWorkDetailList.invoice_seekers.map((x, index) => {
				...
				return {
					...
					misb_commission: store.getters['teamInvoice/editCalculateMISBCommisionOfSeeker'](index),
					w2w_commission: store.getters['teamInvoice/editCalculateW2WCommisionOfSeeker'](index),
					misb_commission_per: store.state.teamInvoice.editTeamWorkDetailList.misb_commission,
			...
			// values.payment_sharing = store.state.teamInvoice.editPaymentSharing;
			...
			await store
				...
				.catch(err => {
					console.log(err);
					Swal.fire({
						text: 'Add Proper Work Deatils And Then After save the Invoice',
						icon: 'warning',
