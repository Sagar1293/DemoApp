	...
	<div class="invoice__list card-space">
		...
		<div class="text-center mt-5 pb-3">
			<ToolTip :title="t('toolTip.saveTeam')">
				<button class="btn-common btn-red" @click="submitForm" :disabled="paymentSharingRef?.teamMembers.length === 0 || paymentSharingRef?.leftToShareAmount !== 0">
					{{ t('common.save') }}
				</button>
			</ToolTip>
		</div>
	...
	setup() {
		...
		const invoiceSchema = yup.object({
			...
			companyId: yup
				...
				.required(
					t('validation.requiredField', {
						field: 'invoiceData.employerDetails',
					}),
				),
		...
		const submitForm = handleSubmit(async values => {
			...
			values.team_worked_hours = store.state.teamInvoice.teamWorkDetailList.team_members.map((x, index) => {
				...
				return {
					...
					misb_commission: store.getters['teamInvoice/calculateMISBCommisionOfSeeker'](index),
					w2w_commission: store.getters['teamInvoice/calculateW2WCommisionOfSeeker'](index),
					misb_commission_per: store.state.teamInvoice.teamWorkDetailList.misb_commission,
