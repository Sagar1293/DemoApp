<template>
					...
					<div v-if="expandRowIndex === index">
						<div class="details-content">
							...
							<div class="work-width ml-xl-0">
								<div class="work py-0">
									<input type="text" :v-model="item.description == null ? '' : item.description" @change="e => handleContractValue(e, index, 'description')" class="form-control" />
								</div>
							</div>
							...
							<div class="buttons">
								...
								<ToolTip :title="t('toolTip.deleteRecord')">
									<div class="delete-btn mx-1" :class="{ btnDeletRow: workDetailList.length < 2 }" @click="removeWorkDetail(index, item.id)">
								...
								<ToolTip :title="t('toolTip.tripButton')">
									<div class="down-btn ms-1" :class="{ btnDisabled: workDetailBtnDisabled(index) }" @click="addWorkDetails(item.id, index)">
											...
											<div class="row no-gutters">
												...
												<div class="col-4">
													<ul>
														...
														<li class="dark-box-text lh-lg">€ {{ item.lunchCompensationAmount * item.totalDays || 0 }}</li>

														<!-- full day allowances - amount -->
														<li class="dark-box-text lh-lg">€ {{ item.fullHalfDayCompensationAmount * item.totalDays || 0 }}</li>
					...
					<div v-else>
						<div class="details-content">
							...
							<div class="work-width ml-xl-0 ml-3">
								<div class="work py-0">
									<input type="text" v-model="item.description" @change="e => handleContractValue(e, index, 'description')" class="form-control" />
								</div>
							</div>
							...
							<div class="buttons">
								...
								<!-- expand work detail -->
								<ToolTip :title="t('toolTip.expantiate')">
									<div class="down-btn me-1" v-if="index !== expandRowIndex" @click="expandWorkDetails(index, item)">
										<FontAwesomeIcon :icon="downIcon" />
									</div>
								</ToolTip>
								...
								<ToolTip :title="t('toolTip.tripButton')">
									<div class="down-btn me-1" :class="{ btnDisabled: workDetailBtnDisabled(index) }" @click="addWorkDetails(item.id, index)">
										<FontAwesomeIcon :icon="euroSign" />
									</div>
								</ToolTip>

								<!-- <ToolTip title="Save button">
									<div class="copy-btn" @click="updateWorkDetailInfo(index)">
										<FontAwesomeIcon :icon="saveIcon" />
									</div>
								</ToolTip> -->
							</div>
								...
								<div class="row align-items-end">
									...
									<div class="col-sm-2 col-5 pe-0 ps-lg-2 ps-1">
										<div class="buttons d-flex align-items-center justify-content-sm-between">
											...
											<!-- remove work detail -->
											<ToolTip :title="t('toolTip.deleteRecord')">
												<div class="delete-btn" :class="{ btnDeletRow: workDetailList.length < 2 }" @click="removeWorkDetail(index, item.id)">
													<FontAwesomeIcon :icon="trashIcon" />
												</div>
											</ToolTip>

											<ToolTip :title="t('toolTip.tripButton')">
												<div class="down-btn" :class="{ btnDisabled: workDetailBtnDisabled(index) }" @click="addWorkDetails(item.id, index)">
													<FontAwesomeIcon :icon="euroSign" />
												</div>
											</ToolTip>
										</div>
			...
			<div class="bottom-header d-lg-flex align-items-center justify-content-between">
				...
				<div class="d-sm-flex align-items-center ml-md-auto flex-md-wrap flex-wrap">
					...
					<!-- <div class="text-white">
							...
							<p class="text-md-fw">{{ dateFormat(formFieldsData.value.dueDateField.value || '-') }}</p>
						</div> -->
	...
	<Suspense v-if="workDeatilsModal">
		<template #default>
			<work-details-modal v-model:isModalShow="workDeatilsModal" :indexId="indexId" :addNewItem="addNewItem" @modalFlag="updateFlag" @workDetails="workDetails" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
...
export default {
	...
	setup(props) {
		...
		// 	return store.state.invoice.vatHandler;
		// });
