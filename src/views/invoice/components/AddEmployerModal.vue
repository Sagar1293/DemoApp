			...
			<div class="modal__content">
				...
				<div class="row d-flex align-items-center justify-content-center m-puzzle" v-if="puzzle">
					...
					<div class="modal-header">
						<h5 class="title-lg">
							{{ t('leInvoice.addInvoice.recipientType') }}
						</h5>
						<!-- <div>
						<label for="switchEmp" class="switch d-flex align-items-center m-0"
							>{{ isPrivateField.value ? 'Private' : 'Company' }}

							<input class="form-check-input form-checkbox-width" type="checkbox" id="switchEmp" @change="switchToggle" :value="true" />
						...
						<a class="header-right-dash" :class="{ userStatusActive: userCompany }" @click="switchToggle('company')">
							<div class="danger-box profession-box-common p-h">
								<span class="prof-bg-puzzle">
									<!-- <img src="~@/assets/images/grey-1-recipient.svg" alt="err" class="" /> -->
								...
								<div class="text-content d-flex flex-column justify-content-between h-100 w-100">
									...
									<div class="d-flex">
										<!-- <div class="sign-up-img-wrap ml-auto">
											<img src="~@/assets/images/magnifying_glass_front.png" alt="err" class="img-fluid" />
										</div> -->
									</div>
								</div>
							</div>
					...
					<div class="col-xl-6 col-lg-6 col-sm-6 col-12 p-0">
						<a class="header-right-dash" :class="{ userStatusActive: userPrivate }" @click="switchToggle('private')">
							<div class="danger-box profession-box-common p-h">
								<span class="prof-bg-puzzle">
									<!-- <img src="~@/assets/images/grey-2-recipient.svg" alt="err" class="" /> -->
									<svg xmlns="http://www.w3.org/2000/svg" class="recipient-puzzle" width="710" height="710" viewBox="0 0 710 710">
								...
								<div class="text-content d-flex flex-column justify-content-between h-100 w-100">
									...
									<div class="d-flex">
										<!-- <div class="sign-up-img-wrap ml-auto">
											<img src="~@/assets/images/megaphone_front.png" alt="err" class="img-fluid" />
										</div> -->
									</div>
								</div>
							</div>
						...
						<div class="row mt-3 m-0">
							<div class="col-12">
								<form class="theme-input">
									<p class="mb-3 col-xl-12 fw-bold p-0">{{ t('leInvoice.addInvoice.basicInformation') }}:</p>
									...
									<p class="mb-3 col-xl-12 fw-bold p-0">{{ t('leInvoice.addInvoice.contactDtlInvAddress') }}:</p>
									<div class="form-row">
							...
							<div class="d-flex align-items-center justify-conent-center">
								<!-- submit btn -->
								<button type="button" class="btn-common btn-red mt-2 w-25 mx-auto" @click="submitForm">
									{{ t('common.submit') }}
								</button>
							</div>
						</div>
				...
				<div>
					<div v-if="form === 'private'">
						...
						<div class="row mt-3 m-0">
							<div class="col-12">
								<form class="theme-input">
									<p class="mb-2 col-xl-12 p-0 fw-bolder">{{ t('leInvoice.addInvoice.basicInformation') }}:</p>
									...
									<div class="form-row">
										<p class="mb-2 col-xl-12 p-0 fw-bolder">{{ t('leInvoice.addInvoice.contactDtlInvAddress') }}:</p>
										...
										<div class="form-group col-12">
											<label for="descripation">
												{{ t('fieldList.descripation') }}
											</label>

											<textarea
												rows="4"
												class="explaination"
												placeholder="Type Your descripation"
	...
	setup(props, { emit }) {
		...
		const employerSchema = yup.object({
				...
				.when('is_private', is_private => {
					if (is_private === true) {
						return yup
							...
							.typeError('Full name is a required field')
			...
			mobile: yup
				...
				.matches(phoneRegExp, 'Phone Number code is not valid')
				.max(16),
			reg_number: yup
				.string()
				.nullable()
			...
			description: yup
				...
				.when('is_private', is_private => {
					if (is_private === true) {
						return yup
							.string()
							.typeError('Descripation is a required field')
							.required(
								t('validation.requiredField', {
			...
			contractName: yup
				...
				.when('is_private', is_private => {
					if (is_private === false) {
						return yup
							.string()
							.matches(/^([A-Za-zÀ-ÖØ-öø-ÿ]+\s){1,2}([A-Za-zÀ-ÖØ-öø-ÿ]+)$/, 'Contact name is not valid')
							.typeError('Contact name is a required field')
		...
		const submitForm = handleSubmit(async values => {
			await store.dispatch('auth/addAnonymousEmployer', { ...values, vat_number: `${countryCode.value}-${values.vat_number}` }).then(() => {
		...
		// isPrivateField.value = val.target.checked;
		...
		// const removeCompanyCountry = event => {
		// 	if (event) {
		...
		//remove value - Country
		// const removePrivateCountry = event => {
		// 	if (event) {
		// 		countryField.value = null;
		// 	}
		// };
