	...
	<div class="deducation__list pt-3">
		...
		<div class="row">
			...
			<div class="payment-pill-header py-3 text-left">
				<h5 class="title-lg">{{ t('invoiceData.deducationList') }}</h5>
			</div>

			<!-- Deductions List -->
			<!-- <new-data-table :data="deductionListItem || []" class="large-table">
				<template #thead>
					<new-tb-th sort-key="created_at">PaymentTerms</new-tb-th>
					<new-tb-th sort-key="due_date">PartAmount</new-tb-th>
					<new-tb-th sort-key="invoice_number">DeducationNotes</new-tb-th>
				</template>

				<template #data="scopedData" ref="tableBody">
					<new-tb-tr class="invoice__row" :key="i" v-for="(tr, i) in scopedData.data">
						<new-tb-td>{{ tr.payment_term || '-' }}</new-tb-td>
						<new-tb-td>{{ Number(tr.parts_amount).toFixed(2) || '-' }}</new-tb-td>
						<new-tb-td>{{ tr.deduction_notes || '-' }}</new-tb-td>
					</new-tb-tr>
				</template>
			</new-data-table> -->
				...
				<div class="col-12 px-0">
					<div v-if="deductionListItem.length > 0">
						<div class="deducation-box d-md-flex justify-content-between align-items-start p-4">
							<div>
								<h5 class="text-white mb-2">{{ t('leInvoice.addInvoice.deductions') }}</h5>
								<p class="fn15 mb-md-0 mb-2">{{ t('leInvoice.addInvoice.dedDesc') }}</p>
							</div>
							<div class="poduct-wrapper">
								<div v-for="(row, index) in deductionListItem || []" :key="index">
									<div class="mb-1 d-flex justify-content-between">
										<p class="product-name">{{ row.deduction_notes || '-' }} ({{ Number(row.total_parts - row.deducted_parts) }} {{ t('leInvoice.addInvoice.left') }})</p>
