												...
												<div class="pph-left-box">
													<div class="row">
														...
														<div class="col-md-12">
															<div class="col-md-12 form-group p-0">
																<div class="position-relative">
																	<label for="endingAddress" class="company-titel">{{ t('leInvoice.addInvoice.endingAddress') }}</label>
														...
														<label for="firstName" class="company-titel pl-3">{{ t('leInvoice.addInvoice.extras') }} </label>
														<div class="ml-3">
															<div class="row">
														...
														<div class="col-md-4 col-6">
															...
															<!-- <input style="border: none" id="calculatedtripTime" type="text" :value="calculateTripTime" disabled /> -->
														</div>
									...
									<div class="detail-sec">
										...
										<div class="invoice-grey-sec">
											<div class="row">
												...
												<!-- <div class="col-md-3 form-group">
													<label for="tripName" class="company-titel"> Value </label>
													<input
														id="tripName"
														type="number"
														min="0"
												...
												<!-- <div class="col-md-3 form-group">
													<label for="firstName" class="company-titel">Date</label>
													<div class="form-group dropdown select-employee">
											...
											<div class="text-secondary summary-text">
												{{ t('leInvoice.addInvoice.summary') }} = €{{ getLunchCompensationValue }}
												<span class="text-small"> ({{ t('leInvoice.addInvoice.lunchCompensation') }})</span> * €{{ daysDiff }}
												<span class="text-small">({{ t('leInvoice.addInvoice.tripDays') }}) </span> = €{{ Number(getLunchCompensationValue) * Number(daysDiff) }}
											</div>
										</div>
									</div>
									<!-- End Vehicle Section -->
...
import { vehicleClassTypes, vehicleSubClassTypes, vatTypeOptionsNew, contractTypeOptions, gAuComRestriction, invoiceAllownceTracer } from '@/utils/enums.js';
...
export default {
	...
	setup(props, { emit }) {
		...
		const wayPointDestination = computed(() => {
			...
			// const waypts = [];
			for (let i = 0; i < wayPointDestination.value.length; i++) {
				console.log(i, '<><><><><><><>');
				// 	if (checkboxArray.options[i].selected) {
				// 		console.log(checkboxArray.options[i].value, '<><><><><><><>');
				// 		waypts.value.push({
		...
		const submitForm = handleSubmit(async values => {
			...
			if (invoiceWorkDetails.length) {
				...
				// workDetailData.address_mid_points = `${start.value}, ${wayPointItems.value.map(x => x.location).toString()} , ${end.value}`;
				...
				// workDetailData.dayCompensationAmount = values.day_compensation == 0 ? 0 : (halfFullCompnesation.value.find(x => x.value === values.day_compensation) || {}).compensation_value;
		...
		const onTrailer = () => {
			...
			// onTrailerValue.value = invoiceAllownceTracer.driven_in_rough_terrain;
		...
		const fetchInvoiceWorkDairies = () => {
			...
			// document.getElementById('totalDistance').innerHTML = dataWorkDetail.distance || 0;
