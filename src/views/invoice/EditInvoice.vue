<template>
	...
	<div class="invoice__list card-space">
		...
		<div class="text-center mt-5 pb-3">
			<button type="button" class="btn-common btn-red" @click="updateInvoice">{{t('common.update')}}</button>
		</div>
		</ToolTip>
	</div>
</template>
...
export default {
	...
	setup() {
		document.title = 'Edit invoice - Way2Work'
		// ref
		...
		onMounted(() => {
			...
			store
				...
				.then(({ data }) => {					
					...
					// 	let fileData = {};
					// });
					// receiptFileData.values = editInvoiceData.invoice_files.reduce((acc, item) => {
					// 	acc.push({
					// 		purchase_place: item.purchase_place,
					// 		purchase_date: item.purchase_date,
					// 		receipt_file: item.file_path,
					// 	});
					// 	return acc;
					// }, []);
