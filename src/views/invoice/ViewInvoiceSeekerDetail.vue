		...
		<div>
						...
						<div class="pph-left-box large-table">
							...
							<div class="company-content">
								...
								<!-- <div class="company-text">
										...
										<p class="company-value">
											{{ seekerInvoiceDetail.company && seekerInvoiceDetail.company.reg_number || '-' }}
										</p>
									</div> -->
						...
						<div class="red-box">
							...
							<div>
								...
								<div class="mb-2" v-if="seekerInvoiceDetail.invoice_company && seekerInvoiceDetail.invoice_company.reg_number">
									<h5 class="title-lg mb-1">{{ t('leInvoice.viewInvoice.companyID') }}</h5>
									<p class="text-md-fw">
										{{ seekerInvoiceDetail.invoice_company && seekerInvoiceDetail.invoice_company.reg_number }}
									</p>
								</div>
			...
			<div v-if="seekerInvoiceDetail.under_review">
				<div class="pph-left mb-3">
					<h5 class="title-head">{{ t('leInvoice.viewInvoice.revisionNote') }}</h5>
				</div>
			...
			<div>
				...
				<new-data-table :data="seekerInvoiceDetail.worked_hours || []" sst class="large-table">
					<template #thead>
						...
						<new-tb-th class="text-center">{{ t('leInvoice.addInvoice.vat') }}%</new-tb-th>
					...
					<template #data="scopedData" ref="tableBody">
						<new-tb-tr class="invoice__row" :key="i" v-for="(tr, i) in scopedData.data">
							...
							<new-tb-td class="text-center">{{ seekerInvoiceDetail.vat || '-' }}</new-tb-td>

							<new-tb-td>€ {{ tr.contract_value * tr.total_work }}</new-tb-td>

							<new-tb-td>€ {{ (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100 }} </new-tb-td>
							...
							<new-tb-tr class="invoice__row expand-table " :key="i" v-for="(tr, i) in scopedData.data">
								...
								<div class="border-padding">
									<div v-if="i == tableIndex" class="table-border">
										<new-tb-tr>
											<new-tb-th>{{ t('leInvoice.viewInvoice.invoiceDetails') }}</new-tb-th>
											<new-tb-td>{{ tr.description }}</new-tb-td>
										</new-tb-tr>
				...
				<div class="overflow-auto">
					<div class="bottom-header d-lg-flex align-items-center justify-content-between mb-4">
						<!-- <div class="save-icon">
						...
						<div class="d-sm-flex align-items-center ml-md-auto flex-md-wrap flex-wrap">
							...
							<div class="text-white">
								<p class="text-sm-fw">{{ t('leInvoice.addInvoice.willpaid') }}</p>
								<p class="text-sm-fw">
					...
					<div class="bottom-header d-lg-flex align-items-center justify-content-between mb-4">
						...
						<div class="d-sm-flex align-items-center ml-md-auto flex-md-wrap flex-wrap">
							...
							<!-- <div class="text-white">
								...
								<p class="text-md-fw">{{ dateFormat(seekerInvoiceDetail.due_date || '-') }}</p>
							</div> -->
					...
					<div class="viewinvoicetable">
						<div class="d-flex">
							...
							<div style="width:55%; border-bottom:2px solid #000;" class="p-2">
								<p>BIC</p>
								<p class="ml-2">Bank BIC : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.bic_number }}</p>
							</div>
						</div>
						...
						<div class="d-flex">
							...
							<div class="d-flex flex-column justify-content-between p-2" style="width:33%; border-right:2px solid #000;">
								<div class="p-2">
									Customer details
									<p>Full Name : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.user && seekerInvoiceDetail.seeker.user.full_name }}</p>
									<p>Email : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.user && seekerInvoiceDetail.seeker.user.email }}</p>
									<p>Phone Number : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.user && seekerInvoiceDetail.seeker.user.mobile }}</p>
									<p>Gender : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.user && seekerInvoiceDetail.seeker.user.gender }}</p>
									<p>DOB : {{ seekerInvoiceDetail.seeker && seekerInvoiceDetail.seeker.user && dateFormat(seekerInvoiceDetail.seeker.user.dob) }}</p>
								</div>
							...
							<div style="width:55%;border-top:2px solid #000;" class="d-flex align-items-end">
								...
								<div style="width:86%;border-top:2px solid #000;" class="p-2">Reference Number : {{ seekerInvoiceDetail.ref_number }}</div>
							</div>
						</div>
						...
						<div style="border-top:2px solid #000;">
							<div class="d-flex">
								...
								<div class="p-2" style="width:25%; border-bottom:2px solid #000;">
									<p>Summa</p>
									<p class="text-center">Total : {{ seekerInvoiceDetail.sub_total }}</p>
								</div>
							</div>
						</div>
...
export default {
	...
	setup() {
		...
		watch(seekerInvoiceDetail, () => {
			...
			// console.log(moment(seekerInvoiceDetail.value.due_date).fromNow(),'days.value');
		...
		const printDocument = () => {
			console.log('gashjfgasdfjagsjdf');
			//get table html
