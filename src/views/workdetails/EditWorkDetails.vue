	...
	<div class="invoice__list card-space">
		<form class="theme-input tab-form">
			<div class="invoice-sec">
				<div class="row">
					...
					<div class="col-xl-6 mt-4">
						...
						<div class="pph-left">
							<h5 class="title-lg mb-2">{{ t('leInvoice.addInvoice.tripDtl') }}</h5>
						</div>
						<div class="pph-left-box">
							<div class="row">
								<!-- trip Name -->
								<div class="col-md-12 form-group">
									<label for="editTeamTripName" class="company-titel"> {{ t('fieldList.tripName') }} </label>
									<input
										id="editTeamTripName"
										type="text"
										class="form-control"
									...
									<div class="form-group p-0">
										<div class="position-relative">
											<div class="d-flex justify-content-between align-items-end mb-1">
												<label for="startingAddress" class="company-titel">{{ t('leInvoice.addInvoice.startingAddress') }}</label>
												<div class="plus">
								...
								<div class="col-md-12">
									<div class="col-md-12 form-group p-0">
										<div class="position-relative">
											<label for="midpoints" class="company-titel">{{ t('leInvoice.addInvoice.midpointsAddress') }}</label>
								...
								<div class="col-xxl-6 col-xl-12">
									<div class="form-group p-0">
										<div class="position-relative">
											<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.endingAddress') }}</label>
							...
							<div class="">
								...
								<div v-if="showTeamField.value" class="row me-0">
									...
									<div class="col-md-4 pe-0">
										<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.tripStartingDate') }}</label>
									...
									<div class="col-md-8 pe-0">
										<div class="form-group p-0">
											<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.tripStartingTime') }}</label>
									...
									<div class="col-md-4 pe-0">
										<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.tripEndingDate') }}</label>
										<div class="form-group dropdown select-employee">
											<p class="company-input input-date">
												<v-date-picker
													@input="tripStartDateField.handleChange;"
									...
									<div class="col-md-8 pe-0">
										<div class="form-group p-0">
											<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.endingTime') }}</label>
											<div>
												<div class="row m-0">
													<div class="col-12 p-0">
														<div class="dropdown select-employee">
							...
							<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.extras') }} </label>
							<div class="ml-3">
								<div class="row">
									<div class="col-md-4 form-check form-checkbox">
										...
										<label class="form-check-label extras-font checkbox-left-padding white-space" for="editWorkTransportedHeavyLoad">
											{{ t('leInvoice.addInvoice.heavyLoad') }}
										</label>
									</div>
									...
									<div class="col-md-4 form-check form-checkbox">
										...
										/>
										<label class="form-check-label extras-font checkbox-left-padding white-space" for="drivenRoughTerrain"> {{ t('leInvoice.addInvoice.roughTerrain') }} </label>
									</div>
									...
									<div class="col-md-12 col-6 px-0">
										<!-- <label for="firstName" class="company-titel">Extras </label> -->
										<div class="mt-2">
											<div class="form-check form-checkbox">
												...
												<label class="form-check-label extras-font checkbox-left-padding" for="multipleDays"
													>{{ t('leInvoice.addInvoice.multipleRowsBy') }} {{ daysDiff == 1 ? 0 : daysDiff || 0 }} {{ t('workDetails.days') }}
												</label>
											</div>
										</div>
									</div>
							...
							<div class="row">
								...
								<div class="col-md-4 col-6">
									<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.totalDistance') }}</label>
									<p class="trip-text" id="totalDistance" style="display: none;"></p>
									<p class="trip-text">{{ distanceKillometer }} {{ t('common.km') }}</p>
								</div>

								<div class="col-md-4 col-6">
									<label for="firstName" class="company-titel">{{ t('leInvoice.addInvoice.totalAllowances') }}</label>
									<p class="trip-text" id="totalDistance" style="display: none;"></p>
					...
					<div class="col-xl-6 mt-lg-4 mt-5">
						<div class="pph-left">
							<h5 class="title-lg mb-2">{{ t('leInvoice.addInvoice.preview') }}</h5>
						</div>
					...
					<div class="row">
						...
						<div class="col-xl-3 col-md-6 form-group mb-3">
							...
							<div>
								...
								/>
								<!-- @click="removedayCompensationField" -->
							</div>
						</div>
						<div class="col-xl-3 col-md-6 form-group mb-3">
			...
			<div class="invoice-pd invoice-receipt">
				<div class="row" v-for="(row, index) in receiptItem" :key="row">
					...
					<div class="col-xl-6 col-6 form-group">
						<label for="firstName" class="input-title">Place of Purchase</label>
					...
					<div class="col-xl-6 col-6 theme-input">
						<label for="firstName" class="input-title">Date of Purchase</label>
						<v-date-picker
			...
			<div class="invoice-pd add-row">
				<div class="pph-left">
					<!-- <p class="title-sm add-text">You can add new row by clicking + in the right bottom corner</p> -->
				</div>
				<span class="border-top w-100 mx-2"></span>
				<!-- <div class="plus ml-auto" @click="addReceiptDetail">
			...
			<div class="text-center mt-5 pb-3">
				<ToolTip :title="t('toolTip.updateWorkDetail')">
					<button type="button" class="btn-common btn-red mt-2" @click="submitForm">{{ t('common.update') }}</button>
				</ToolTip>
			</div>
		</form>
	</div>
...
import { vehicleClassTypes, vehicleSubClassTypes, vatTypeOptionsNew, contractTypeOptions, invoiceAllownceTracer, gAuComRestriction } from '@/utils/enums.js';
...
export default {
	...
	setup() {
		document.title = 'Edit Work Detail - Way2Work';
		// ref
		...
		const workDetailsSchema = yup.object({
			...
			total_work: yup
				.number()
				.typeError('Hours / Pieces is a required field')
				.nullable()
				.required(
		...
		const { meta: formMeta, handleSubmit, isSubmitting, errors } = useForm({
		...
		onMounted(() => {
			...
			// receiptItem.value = store.state.workDetail.workDetail.receipts.map((x) => ({
			// 	id: x.id,
		...
		const calculateTripTime = computed(() => {
			...
			// const endTime = tripEndTime.value;
			// const startDate = moment(tripStartDateField.value).format('YYYY/MM/DD');
			// const endDate = moment(tripStartDateField.value).format('YYYY/MM/DD');
		...
		const calculateFileSize = computed(() => {
			return file => {
				// receiptItem.value[index].receipt_file = imageFile[0];
		...
		const submitForm = handleSubmit(async values => {
			...
			// values.receipts = receiptItem.value || null;
		...
		const removeWayPointRow = index => {
			wayPointItems.value.splice(index, 1); // why is this removing only the last row?
			const startingAddr = {
		...
		//remove value - Show In Terms
		// const removeTerm = event => {
		// 	if (event) {
		// 		showTeamField.value = null;
		// 	}
		// };
		...
		// 	if (event) {
		// 		contactTypeField.value = null;
		// 	}
		// };

		//remove value - Lunch Compensation
		// const removeLunchCompensation = event => {
		// 	if (event) {
		// 		lunchCompensationField.value = null;
		// 	}
		// };

		//remove value - lunch Compensation Field
		// const removedayCompensationField = event => {
		// 	if (event) {
		// 		dayCompensationField.value = null;
		// 	}
		// };
		...
		return {
			...
			// removeTerm,
			// removeContractType,
			// removeLunchCompensation,
			// removedayCompensationField,
		};
	},
};
</script>
