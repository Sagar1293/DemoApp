				...
				<div class="modal-form">
					...
					<form class="theme-input tab-form">
						<div class="form-row">
							...
							"maxTaxPercentage": "Max Tax Percentage", -->
	...
	setup(props, { emit }) {
		...
		const documentSchema = yup.object({
			...
			document_name: yup
				.string()
				.nullable()
				.typeError('Document name must be at least 2 characters')
				.max(60)
				.required(
					t('validation.requiredField', {
						field: 'fieldList.documentName',
				...
				.when('document_type', document_type => {
					if (document_type === taxCardId.value) {
						return yup
							.number()
							.nullable()
							.typeError('Yearly Income must be enter number')
							.required(
								t('validation.requiredField', {
									field: 'fieldList.yearlyIncome',
								}),
				...
				.when('document_type', document_type => {
					if (document_type === taxCardId.value) {
						return yup
							.number()
							.nullable()
							.typeError('Min Personal Tax must be enter number')
							.required(
								t('validation.requiredField', {
									field: 'fieldList.personalTax',
								}),
				...
				.when('document_type', document_type => {
					if (document_type === taxCardId.value) {
						return yup
							.number()
							.nullable()
							.typeError('Max Personal Tax must be enter number')
							.required(
								t('validation.requiredField', {
									field: 'fieldList.maxTaxPercentage',
								}),
			...
			start_date: yup
				.date()
				.nullable()
				.typeError('Plase Select proper start date')
				.required(
					t('validation.requiredField', {
			...
			expiry_date: yup
				.date()
				.nullable()
				.typeError('Plase Select proper expiry date')
				.required(
					t('validation.requiredField', {
		...
		const submitForm = handleSubmit(async values => {
			...
			await store.dispatch('document/uploadDocumentFile', formData).then(() => {
		...
		//remove Value - Document Type
		// const removeDocType = event => {
		// 	if (event) {
