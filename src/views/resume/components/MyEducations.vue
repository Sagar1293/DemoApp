<template>
	<div class="my_educations">
		<div class="payment-pill-header">
			...
			<div class="pph-right">
				<ToolTip :title="t('toolTip.addmyeducations')">
					<button class="btn-common btn-red" type="button" @click="showAddEducationModal = true">+ {{
							t('common.add')
					}}</button>
				</ToolTip>
			</div>
		...
		<new-data-table :data="educationList" sst pagination search @search="updateSearchQuery"
			:total="totalEducationCount" :max-items="maxItemsPerPage" @change-page="handleChangePage"
				...
				<div class="select__limit d-flex align-items-center">
					...
					<div class="dropdown mx-2">
						<a class="
									...
									tbl-drop
								" href="#" id="navbarDropdownEducation" role="button" data-bs-toggle="dropdown" aria-expanded="false">
							{{ maxItemsPerPage }}
						</a>
			...
			<template #thead>
				<!-- <new-tb-th sort-key="qualification">{{ t('fieldList.degreeTitle') }}</new-tb-th> -->
			...
			<template #data="scopedData" ref="tableBody">
				<new-tb-tr class="invoice__row" :key="i" v-for="(tr, i) in scopedData.data">
					<!-- <new-tb-td>{{ tr.qualification?.qualification }}</new-tb-td> -->
					...
					<div class="select__limit d-flex align-items-center">
						...
						<div class="dropdown mx-2">
							<a class="
								...
								btn-common btn-gray-border
								tbl-drop
							" href="#" id="navbarDropdownEducationMobile" role="button" data-bs-toggle="dropdown" aria-expanded="false">
								{{ maxItemsPerPage }}
							</a>
											...
											<div class="d-flex align-items-center justify-content-center">
												<ToolTip :title="t('toolTip.editmyeducations')">
													<button class="tbl-action grey-edit"
														@click="EditEducationDetail(tr.id)">
	...
	<Suspense v-if="showEditEducationModal.value">
		<template #default>
			<edit-education v-model:isModalShow="showEditEducationModal" v-model:educationId="educationId"
				@newEducation="fetchEducations" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
...
export default {
	...
	setup() {
		...
		const deleteEducation = id => {
			Swal.fire({
				title: 'Are you sure?',
				text: 'You want to delete education ?',
				icon: 'warning',
