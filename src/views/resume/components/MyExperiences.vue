<template>
	<div class="my_experiences">
		<div class="payment-pill-header">
			...
			<div class="pph-right">
				<ToolTip :title="t('toolTip.addmyexperience')">
					<button class="btn-common btn-red" type="button" @click="showAddexperienceModal = true">+ {{
							t('common.add')
					}}</button>
				</ToolTip>
			</div>
		...
		<new-data-table :data="experienceList" sst pagination search @search="updateSearchQuery"
			:total="totalExperinceCount" :max-items="maxItemsPerPage" @sort="handleSort" @change-page="handleChangePage"
			class="large-table">
			<template #header>
				<div class="select__limit d-flex align-items-center">
					...
					<div class="dropdown mx-2">
						<a class="
									...
									tbl-drop
								" href="#" id="navbarDropdownExperience" role="button" data-bs-toggle="dropdown" aria-expanded="false">
							{{ maxItemsPerPage }}
						</a>
		...
		<div>
			<new-data-table :data="experienceList" sst pagination search @search="updateSearchQuery"
				:total="totalExperinceCount" :max-items="maxItemsPerPage" @change-page="handleChangePage"
					...
					<div class="select__limit d-flex align-items-center">
						...
						<div class="dropdown mx-2">
							<a class="
								...
								btn-common btn-gray-border
								tbl-drop
							" href="#" id="navbarDropdownExperienceMobile" role="button" data-bs-toggle="dropdown" aria-expanded="false">
								{{ maxItemsPerPage }}
							</a>
											...
											<div class="d-flex align-items-center justify-content-center">
												<ToolTip title="Edit my experience">
													<button class="tbl-action grey-edit"
														@click="EditExperienceDetail(tr.id)">
	...
	<Suspense v-if="showEditexperienceModal.value">
		<template #default>
			<edit-experience v-model:isModalShow="showEditexperienceModal" v-model:experiencId="experiencId"
				@newExperience="fetchExperiences" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
