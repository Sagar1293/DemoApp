<template>
	<div class="job__proposals car-space">
		<!-- job proposals list -->
		<div class="spacing-table table-responsive table-auto large-table card-space">
			<!-- header -->
			<div class="payment-pill-header">
				<!-- title -->
				<div class="pph-left mb-3">
					<h5 class="title-lg">
						{{ t('jobProposal.teamMember') }}
					</h5>
				</div>
			</div>
			...
			<div class="table-width">
				<div class="spacing-table-head">
					<div>{{ t('jobProposal.lightEnterpreneurName') }}</div>
						...
						<div class="border-padding">
							<div v-if="i == tableIndex" class="table-border">
								<div class="d-flex align-items-center px-sm-4 px-2 py-3 border-bottom bg-white">
									<div class="min-width w-100 text-xs">{{ t('jobProposal.lightEnterpreneurName') }}</div>
	...
	<!-- Add team memberform -->
	<Suspense v-if="showAddTeamMemberModal.value">
		<template #default>
			<add-team-member v-model:isModalShow="showAddTeamMemberModal" @addNewTeamWorkDetail="fetchTeammembers" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
	</Suspense>
	<!-- Edit team memberform -->
</template>
...
const AddTeamMember = defineAsyncComponent(async () => {
	return import('@/views/profile/components/AddTeammemberModal.vue' /* webpackChunkName:"UploadMyDocument" */);
});
...
export default {
	...
	setup() {
		document.title = 'Team member list - Way2Work';
		// refs
		const page = ref(1);
