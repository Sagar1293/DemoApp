<template>
	<!-- update profile -->
	<div class="invoice__list account__form">
		<!-- profile form -->
		<form class="tab-form theme-input">
			<!-- title -->
			<div class="form-group col-12 p-0">
				<!-- <h5 class="title-lg">{{ t('updateProfile.title') }}</h5> -->
				<div class="d-flex align-items-center justify-content-between mb-4 flex-md-nowrap flex-wrap">
					<h5 class="title-lg mb-md-0 mb-2">{{ t('sidebar.myProfile') }}</h5>
					<label class="switch d-flex align-items-center m-0">
						<h6 class="fn15">{{ t('leProfile.leAccount.gigAvaMsg') }}</h6>
				...
				<div class="d-flex align-items-center justify-content-sm-between justify-content-center flex-lg-nowrap flex-wrap">
					<div class="d-flex align-items-center">
						<span v-for="(item, index) in 5" :key="index">
							<div class="ml-lg-2">
								...
								<!-- <button>
									<FontAwesomeIcon :icon="startIcon" class="w-23 h-23" />
								</button> -->
							</div>
						</span>
					</div>

					<!-- @click="viewSeekerDocument(docFilePic)" -->
					<div class="d-flex flex-md-nowrap flex-wrap lign-items-center justify-content-lg-start justify-content-center mt-lg-0 mt-3">
						<button class="btn-common btn-red py-2 mb-md-0 mb-2" type="button" @click="viewShowTaxCardModal">{{ t('leProfile.leAccount.viewTaxCards') }}</button>

						<button class="btn-common btn-red py-2 mb-md-0 mb-2 ms-sm-3 ms-2" type="button" @click="redirectDocument">{{ t('commanDashboard.myDocuments') }}</button>
			...
			<div class="update-profile-card p-sm-4 p-2 mt-1 mb-3">
				<div class="form-group mb-0 d-lg-flex main-profile-card">
					...
					<!-- <div class="form-group">
						<div class="drop-container" v-show="false">
					...
					<div class="form-group col-12 p-0" v-show="false">
						<label for="uploadCompanyLogo">{{ t('registerForm.uploadCompanyLogo') }}</label>
					...
					<div class="card-space mb-0 ms-lg-3 px-sm-4 px-3 py-0 z-1">
						<div class="row">
							<div class="col-xxl-9">
								<h5 class="text-big">{{ t('leProfile.leAccount.updateProfile') }}</h5>
								<div class="row">
									<!-- gender -->
							...
							<div class="col-xxl-3 relative">
								<div class="d-flex flex-column justify-content-between h-100 bg-img">
									<div class="update-profile-info">
										<h5 class="text-big">{{ t('updateProfile.taxInfoTitle') }}</h5>
										<div>
											<div class="d-flex align-items-center justify-content-between mb-1">
												<div class="tax-title">
													{{ t('fieldList.yearlyIncome') }}
												</div>
												<div class="tax-value">{{ yearyIncome }} €</div>
											</div>
											<div class="d-flex align-items-center justify-content-between mb-1">
												<div class="tax-title">
													{{ t('leProfile.leAccount.withholdingRate') }}
												</div>
												<div class="tax-value">{{ personalTax }} %</div>
											</div>
											<div class="d-flex align-items-center justify-content-between mb-1">
												<div class="tax-title">
													{{ t('leProfile.leAccount.topWithholding') }}
												</div>
												<div class="tax-value">{{ maxTaxPercentage }} %</div>
											</div>
										</div>
									</div>
									<div>
										<p class="text-gray text-end mb-1 mt-sm-0 mt-2 fn15">{{ t('leProfile.leAccount.taxCardUpdatedOn') }} {{ taxDateupdated }}</p>
										<span v-if="userVerify">
											<p class="text-gray text-end uppercase fn16">{{ t('leProfile.leAccount.verifiedUser') }}</p>
										</span>
										<span v-else>
											<p class="text-gray text-end uppercase fn16">{{ t('leProfile.leAccount.not') }} {{ t('leProfile.leAccount.verifiedUser') }}</p>
										</span>
									</div>
								</div>
							</div>
				...
				<div class="row">
					...
					<div class="form-group col-12 p-0 mb-0" @click="bankAccountExpland = !bankAccountExpland">
						<div class="d-flex align-items-center justify-content-between">
							<h5 class="text-big mb-0">
								{{ t('updateProfile.bankAccountTitle') }}
							</h5>
							<div>
...
export default {
	...
	setup() {
		document.title = 'My Profile - Way2Work';
		// ref vars
		...
		const profileSchema = yup.object({
			...
			bank_name: yup
				...
				.matches(stringRegExBank, 'Bank Name is not valid')
				.min(2, 'Bank name must be at least 2 characters')
				.max(60, 'Bank name more then at least 60 characters'),

			account_number: yup
				.string()
				.typeError('Account number number is reqiured.')
				.required(
					t('validation.requiredField', {
			...
			bic_number: yup
				...
				.min(8, 'Bic number must  be at least 8 characters')
				.max(11, 'Bic number must be at most 11 characters'),
		...
		// const docPictureField = reactive(useField('doc_image', undefined, { initialValue: null }));
		...
		// 	console.log(data, 'datraarararararara');
		// };
