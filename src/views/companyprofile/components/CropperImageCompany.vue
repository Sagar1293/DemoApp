	...
	<vue-final-modal v-model="modalShow" classes="modal modal-container upload_contract" content-class="modal-content px-0 pb-0">
		...
		<div class="modal__content">
			...
			<div class="modal-header">
				<h5 class="title-lg">
					<h5 class="title-lg">{{ t('cropImageComapany.image_cropper') }}</h5>
				</h5>
			</div>
				...
				<div class="modal-form">
					...
					<div class="button-wrapper">
						<button class="button" @click="$refs.file.click()">
							...
							{{ t('cropImageComapany.upload_image') }}
						</button>
						<button class="button" @click="cropImage()">{{ t('cropImageComapany.crop_image') }}</button>
					</div>
				</div>
			</div>
		</div>
...
export default defineComponent({
	...
	unmounted() {
		...
		if (this.image.src) {
			// this.$emit('imageUpdated', URL.revokeObjectURL(this.image.src));
		}
	},
