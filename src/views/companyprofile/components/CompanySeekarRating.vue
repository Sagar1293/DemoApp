<template>
	...
	<div class="manage__skil card-space my-3">
		<!-- header -->
		<div>
			<p class="title-lg mb-4">{{ t('companyDashboard.yourReviews') }}</p>
			...
			<div v-if="seekerRatingArray && seekerRatingArray.length > 0">
				<div :key="i" v-for="(item, i) in seekerRatingArray">
					<div>
										...
										<div class="d-flex align-items-center path-width">
											...
											<div class="ms-sm-4 ms-1">
												<p class="mt-sm-0 my-sm-0 my-1 title-base">
													{{ t('companyDashboard.leftReviewTo') }} {{ (item.seeker && item.seeker.user && item.seeker.user && item.seeker.user.full_name) || 'Unnamed user' }}
												</p>
												<p class="mt-sm-0 my-sm-0 my-1 title-base">
						...
						<!-- <div v-else>
							<p>Request access and wait for approval to read the full review</p>
						</div> -->
					</div>
				</div>
			</div>
