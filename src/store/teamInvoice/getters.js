...
export default {
	...
	calculateGrossSalaryOfSeeker: (state, { calculateRemainingWorkValueOfSeeker }) => {
		return (index, vat = localStorage.getItem('teamInvoiceVat') || 0) => {
			return (calculateRemainingWorkValueOfSeeker(index) * (100 + Number(vat))) / 100;
		};
	},
	...
	calculateW2WCommisionOfSeeker: (state, { calculateRemainingWorkValueOfSeeker }) => {
		return (index, vat = localStorage.getItem('teamInvoiceVat') || 0) => {
			return (((calculateRemainingWorkValueOfSeeker(index) * (100 + Number(vat))) / 100) * state.teamWorkDetailList.w2w_commission) / 100;
		};
	},
	...
	calculateQuickPayCommisionOfSeeker: (state, { calculateRemainingWorkValueOfSeeker, getTeamMembersFromTeamImvoice }) => {
		return (index, vat = localStorage.getItem('teamInvoiceVat') || 0, quickPayStat = JSON.parse(localStorage.getItem('quickPayStat')) || false) => {
			...
			return ((calculateRemainingWorkValueOfSeeker(index) * (100 + Number(vat))) / 100) * (getTeamMembersFromTeamImvoice[index].quickpayPercentage / 100);
		};
	},
	 ...
	 * For Add New Invoice
	 */
	calculateCommisionAmountOfSeeker: (state, { calculateMISBCommisionOfSeeker, calculateW2WCommisionOfSeeker, calculateQuickPayCommisionOfSeeker }) => {
		return index => {
			return calculateMISBCommisionOfSeeker(index) + calculateW2WCommisionOfSeeker(index) + calculateQuickPayCommisionOfSeeker(index);
		};
	},
	...
	editCalculateGrossSalaryOfSeeker: (state, { editCalculateRemainingWorkValueOfSeeker }) => {
		return (index, vat = localStorage.getItem('teamInvoiceVat') || 0) => {
			return (editCalculateRemainingWorkValueOfSeeker(index) * (100 + Number(vat))) / 100;
		};
	},
	...
	editCalculateW2WCommisionOfSeeker: (state, { editCalculateRemainingWorkValueOfSeeker }) => {
		return (index, vat = Number(localStorage.getItem('teamInvoiceVat') || 0)) => {
			return ((editCalculateRemainingWorkValueOfSeeker(index) * (100 + vat)) / 100) * (state.editTeamWorkDetailList.w2w_commission / 100);
		};
	},
	...
	editCalculateQuickPayCommisionOfSeeker: (state, { editCalculateRemainingWorkValueOfSeeker, getEditTeamMembersFromTeamImvoice }) => {
		return (index, vat = Number(localStorage.getItem('teamInvoiceVat') || 0), quickPayStat = Boolean(localStorage.getItem('quickPayStat') || false)) => {
			if (!quickPayStat) return 0;
			return ((editCalculateRemainingWorkValueOfSeeker(index) * (100 + vat)) / 100) * (getEditTeamMembersFromTeamImvoice[index].quickpayPercentage / 100);
		};
	},
	 ...
	 * For Edit New Invoice
	 */
	editCalculateCommisionAmountOfSeeker: (state, { editCalculateMISBCommisionOfSeeker, editCalculateW2WCommisionOfSeeker, editCalculateQuickPayCommisionOfSeeker }) => {
		return index => {
			return editCalculateMISBCommisionOfSeeker(index) + editCalculateW2WCommisionOfSeeker(index) + editCalculateQuickPayCommisionOfSeeker(index);
		};
	},
